...
def fetch_barttorvik():
    ...
    response = requests.get("https://www.barttorvik.com/schedule.php", headers=headers)
...
def transform_barttorvik_data(df):
    ...
    base_df = df[['Home Team', 'Away Team']].copy()
...
def get_barttorvik_df():
    """Main function to get processed Barttorvik data"""
    raw_df = fetch_barttorvik()
    transformed = transform_barttorvik_data(raw_df)
    mapped = map_team_names(transformed)
